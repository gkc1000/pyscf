...
from pyscf.pbc.scf_test import conv1D
...
def get_hcore(cell, kpt=np.zeros(3), mf=None):
    '''Get the core Hamiltonian AO matrix.
    '''
    hcore = get_t(cell, kpt)
    if cell.pseudo:
        hcore += ( get_pp(cell, kpt, mf) + get_jvloc_G0(cell, kpt) )
    else:
        hcore += get_nuc(cell, kpt, mf)
...
def get_nuc(cell, kpt=np.zeros(3), mf=None):
...
def get_pp_o1(cell, kpt=np.zeros(3), mf=None):
    '''Get the periodic pseudotential nuc-el AO matrix, with G=0 removed.
    '''
    coords = pyscf.pbc.dft.gen_grid.gen_uniform_grids(cell)
    aoR = pyscf.pbc.dft.numint.eval_ao(cell, coords, kpt)
    ...
    SI = cell.get_SI()
    vlocG = pseudo.get_vlocG(cell)
    Gvnorm = pyscf.lib.norm(cell.Gv, axis=1)
    Gvnorm_ge_0 = Gvnorm > 1e-8
    for ia in range(cell.natm):
        Zia = cell.atom_charge(ia)
        vlocG[ia,Gvnorm_ge_0] -= 4.*np.pi*Zia/(Gvnorm[Gvnorm_ge_0]**2)
        vlocG[ia,:] += Zia*tools.get_coulG(cell,mf=mf)
    vpplocG = -np.sum(SI * vlocG, axis=0)
...
def get_pp(cell, kpt=np.zeros(3), mf=None):
    return get_pp_o1(cell, kpt, mf)
...
def get_vkR_(mf, cell, aoR_k1, aoR_k2, kpt1, kpt2):
    ...
    coulG = tools.get_coulG(cell, kpt1-kpt2, exx=True, mf=mf, G0eq0=False)
...
def ewald(cell, ew_eta, ew_cut, verbose=logger.NOTE):
    ...
    print 'Ewald components = %.15g, %.15g, %.15g, %.15g' %( ewovrl, ewself, ewg , ewovrl + ewself + ewg)
