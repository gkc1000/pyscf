...
def unpack_tril(in_array,nkpts,kp,kq,kr,ks):
    ...
    #print not_tril
    if sum(not_tril) == 0:
        #print "returning normal indices..."
        #print in_array[indices[:,:3]].shape
        #print in_array[[x for x in indices[:,:3]]].shape
        #print in_array[numpy.array([x for x in indices[:,:3]])].shape
    ...
    #print indices.shape
    #print in_array[[x for x in indices[:,:3]]].shape
    # Now we access the array using kp,kq,kr and permute
    # the MO indices if we permuted the kp,kq,kr
    #
    indices = [tril_index(indices[:,0],indices[:,1]),indices[:,2]]
    #print "indices"
    #print indices
